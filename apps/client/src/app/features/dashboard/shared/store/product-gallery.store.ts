...
function normalizeGalleryResponse(body: unknown): IImage[] {
  if (Array.isArray(body)) {
    return body as IImage[];
  }
  if (body && typeof body === 'object' && 'data' in body) {
    const data = (body as { data: unknown }).data;
    return Array.isArray(data) ? (data as IImage[]) : [];
  }
  return [];
}
  ...
  withMethods(({ _http, _toast, ...store }) => ({
    loadAll: rxMethod<string>(
      pipe(
        ...
        switchMap((slug) =>
          _http.get<unknown>(`products/by-slug/${slug}/gallery`).pipe(
            map((body) => {
              patchState(store, { isLoading: false, images: normalizeGalleryResponse(body) });
            }),
            catchError(() => {
